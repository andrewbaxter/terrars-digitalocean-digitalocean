...
struct ReservedIp_ {
    shared: StackShared,
    tf_id: String,
    data: RefCell<ReservedIpData>,
}

#[derive(Clone)]
pub struct ReservedIp(Rc<ReservedIp_>);
...
impl ReservedIp {
    ...
    pub fn set_droplet_id(self, v: impl Into<PrimField<f64>>) -> Self {
        self.0.data.borrow_mut().droplet_id = Some(v.into());
        self
    }
...
impl Resource for ReservedIp {
    fn extract_ref(&self) -> String {
        format!("{}.{}", self.0.extract_resource_type(), self.0.extract_tf_id())
    }
}

impl Dependable for ReservedIp {
    fn extract_ref(&self) -> String {
        Resource::extract_ref(self)
    }
}

impl ToListMappable for ReservedIp {
    type O = ListRef<ReservedIpRef>;
...
impl Resource_ for ReservedIp_ {
...
pub struct BuildReservedIp {
    pub tf_id: String,
    #[doc= ""]
    pub region: PrimField<String>,
}

impl BuildReservedIp {
    pub fn build(self, stack: &mut Stack) -> ReservedIp {
        let out = ReservedIp(Rc::new(ReservedIp_ {
            shared: stack.shared.clone(),
            tf_id: self.tf_id,
            data: RefCell::new(ReservedIpData {
...
pub struct ReservedIpRef {
    shared: StackShared,
    base: String,
}

impl Ref for ReservedIpRef {
