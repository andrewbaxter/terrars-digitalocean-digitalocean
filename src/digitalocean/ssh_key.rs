...
struct SshKey_ {
    shared: StackShared,
    tf_id: String,
    data: RefCell<SshKeyData>,
}

#[derive(Clone)]
pub struct SshKey(Rc<SshKey_>);
...
impl Dependable for SshKey {
    fn extract_ref(&self) -> String {
        Resource::extract_ref(self)
    }
}

impl ToListMappable for SshKey {
    type O = ListRef<SshKeyRef>;
...
impl BuildSshKey {
    pub fn build(self, stack: &mut Stack) -> SshKey {
        let out = SshKey(Rc::new(SshKey_ {
            shared: stack.shared.clone(),
            tf_id: self.tf_id,
            data: RefCell::new(SshKeyData {
